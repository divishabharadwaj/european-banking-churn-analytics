...
def load_and_generate_dataset():
    ...
    rnd = SeededRandom(42)
    customers = list(real_seeds)
    target_count = 10000
    start_id = 15560000
    
    for i in range(len(customers), target_count):
        cust_id = start_id + i
        surname = surnames[int(rnd.next_val() * len(surnames))]
        
        geo_rand = rnd.next_val()
        if geo_rand < 0.50:
            geography = 'France'
        elif geo_rand < 0.75:
            geography = 'Spain'
        ...
        gender = 'Male' if rnd.next_val() < 0.54 else 'Female'
        
        credit_score = int(min(850, max(350, math.floor(650 + (rnd.next_val() - 0.5) * 200 + (rnd.next_val() - 0.5) * 100))))
        ...
        if age_rand < 0.1:
            ...
            age = int(math.floor(60 + rnd.next_val() * 32))
            
        tenure = int(math.floor(rnd.next_val() * 11))
        
        if geography != 'Germany' and rnd.next_val() < 0.40:
            balance = 0.0
        else:
            balance = float(int(math.floor(50000 + rnd.next_val() * 100000 + rnd.next_val() * 70000)))
        ...
        has_cr_card = 1 if rnd.next_val() < 0.70 else 0
        is_active = 1 if rnd.next_val() < 0.51 else 0
        salary = float(int(math.floor(10000 + rnd.next_val() * 190000)))
        ...
        churn_prob = min(0.98, max(0.025, churn_prob))
        exited = 1 if rnd.next_val() < churn_prob else 0
...
# -----------------------------------------------------------------------------
with tab_summary:
    st.markdown("<h3 style='font-family:Playfair Display, serif; italic'>Portfolio Vulnerability Indicators</h3>", unsafe_style_with_html=True)
